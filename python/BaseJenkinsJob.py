...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "BaseJenkinsJob.py <url> <username> <password> <job-name> [--clone | --create]\n")
        sys.exit(-1)
    url = sys.argv[1]
    job_name = sys.argv[4]
    ...
    if op and op == "--clone":
        if not base_jenkins.has_job(job_name):
            sys.exit(
                "Job {} is not a valid job name on server {}".format(job_name, url))
        new_job = input(
            "Please give the new job name to clone from job {} : ".format(sys.argv[3]))
        ...
        base_jenkins_job = BaseJenkinsJob(base_jenkins, job_name)
        base_jenkins_job.create(config_xml)
    else:
        base_jenkins_job = BaseJenkinsJob(base_jenkins, job_name)
